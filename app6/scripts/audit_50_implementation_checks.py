...
def run_audit() -> int:
    """🚪 Выполнить 50 проверок и записать отчёты.

    🔧 FIX (D14): раньше тело выполнялось при импорте — модуль писал файлы
    и завершал процесс через SystemExit. Теперь запуск только явный.
    """
    global results
    results = []
    trees={}
    ...
    try:
        ...
        # Контур переведён на pytest: unittest-discover не видит фикстуры и
        # параметризацию, поэтому раньше сообщал tests=0 при рабочем наборе.
        cp=subprocess.run([sys.executable,"-m","pytest",str(ROOT/"test_module"),"-q"],cwd=package_parent,text=True,capture_output=True,timeout=600,env=env)
        m=re.search(r"(\d+) passed",cp.stdout+cp.stderr); count=int(m.group(1)) if m else 0
    ...
    return 1 if out['fail_count'] else 0
