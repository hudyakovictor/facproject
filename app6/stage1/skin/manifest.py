"""🎯 CRITICAL → Манифест skin-run'а: sha256 входов, создание, финализация.
🚪 API: decoded_sha256(), create_manifest(), finalize_manifest()
🚨 WARNING: стейдж 2 откажется читать пакет без finalize_manifest().
"""
...
# 📊 sha256 декодированного изображения (контроль изменений)
def decoded_sha256(image)->str:return hashlib.sha256(memoryview(image).cast('B')).hexdigest()
# 🏭 FACTORY → создать черновой manifest run'а
...
# 🎯 CRITICAL → финализация: без неё stage2 не читает пакет
