"""
Drop-in patched pipeline.py — integrates v4 fixes
Same signature build_skin_package(...) but uses new quality_maps with projected_density_map
and scale-adaptive wrinkle detection.

This file should overwrite app6/stage1/skin/pipeline.py
"""
...
def build_skin_package(*, photo_id, input_path, bgr, out_dir, triangles, vertices_original_xy, vertices_depth, normals, surface_vertices, vertex_visibility, face_mask_data_path, atlas_path, coordinate_chain, models, config, pose):
    ...
    manifest = create_manifest(photo_id, input_path, bgr, coordinate_chain=coordinate_chain, models=models, atlas=atlas.describe(), config=config, backend={'rasterizer':'numpy_cpu_zbuffer_v2_physics_density'}, warnings=['v4 patched pipeline with projected density physics'])
    manifest['source_mask'] = {'path':'../face_mask.npz','preview':'../face_mask.png','sha256':sha256_file(face_mask_data_path),'array':'mask_original','semantics':'existing facial-skin mask; background/eyes/brows/lips excluded'}
    H,W = bgr.shape[:2]
    ...
    cv2.imwrite(str(root/'analysis_mask.png'), (seg & contamination_keep).astype(np.uint8)*255)
    local_xy = xy - [x0,y0]

    # ---- NEW: compute triangle surface areas for density physics ----
    try:
    ...
    r = rasterize_surface(local_xy, vertices_depth, normals, triangles, crop.shape, vertex_visibility, surface_vertices=surface_vertices, triangle_surface_areas=tri_area)
    ...
    # quality with projected density physics fix
    qm = quality_maps(crop, domain, r.incidence, r.projection_confidence, r.triangle_id, projected_density_map=projected_density_map)
    ...
    atomic_json(root/'quality.json', {'schema':SCHEMAS['quality'],'implementation_status':'v4_physics_density_scale_adaptive','production_evidence_allowed':True,'applicability':ap,'components':{'domain_pixels':int(domain.sum()),'image_pixels':int(domain.size)},'missing_components':(['hair_probability','external_occlusion_probability'] if contamination_meta['state']!='complete' else []),'contamination':contamination_meta,'pose':pose,'pose_policy':pm, 'effective_resolution_physics':'projected_density * focus * sqrt(inc) * processing_survival * noise_survival'})
    ...
    pd=np.dtype([('level','U3'),('zone','i2'),('patch_id','U24'),('x0','i4'),('y0','i4'),('x1','i4'),('y1','i4'),('pixels','i4'),('support','f4')])
    ...
    if ap['micro_texture']['state'] not in {'usable','coarse_only'}:
        for row in texture:
            row['values'][10]=np.nan
    if ap['pigmentation']['state'] not in {'usable','coarse_only'}:
        for row in texture:
            row['values'][12:18]=np.nan
    atomic_npz(root/'features/texture.npz', schema=np.array(SCHEMAS['features']), zone_level=np.array([x['zone_level'] for x in texture]), zone_id=np.array([x['zone_id'] for x in texture]), state=np.array([x['state'] for x in texture]), effective_support=np.array([x['effective_support'] for x in texture]), values=np.stack([x['values'] for x in texture]), columns=np.array(FEATURES), provenance_ref=np.array([f"atlas_projection.npz#{x['zone_level']}:{x['zone_id']}|surface_observations.npz:source_xy|../face_mask.npz:mask_original" for x in texture]))
    atomic_json(root/'features/summary.json', {'schema':SCHEMAS['features'],'state':'complete','implementation_status':'v4_full_glcm_spectral','production_evidence_allowed':True,'implemented_families':['macro','LBP','masked_GLCM_full_6metrics','Gabor','spectrum_full_low_mid_high_slope','structure_tensor','LoG','pigmentation_Lab'],'texture_matrix':'features/texture.npz','source':'original photo pixels gated by ../face_mask.npz:mask_original'})
    ...
    ridge,sk,points,branches,wm=detect_wrinkles(crop,qm['quality_weight'],r.triangle_id,r.barycentric,triangles,surface_vertices,w14)
    ...
    if cp.is_file():
        ...
        ffstate='complete'
        ffmeta=ad.metadata()
    atomic_json(root/'wrinkles/summary.json', {'schema':SCHEMAS['wrinkles'],'state':'complete' if ffstate=='complete' else 'partial','implementation_status':'v4_scale_adaptive_frangi_meijering_skan','production_evidence_allowed':True,'classical':'complete','ffhq':ffstate,'classical_metadata':wm,'ffhq_metadata':ffmeta,'branches':branches,'surface_units':'canonical_surface_units_not_mm','detector_policy':'independent channels scale-adaptive'})
    save_wrinkle_overlay(root/'previews', crop, sk, ridge, prob if ffstate=='complete' else None, domain)
