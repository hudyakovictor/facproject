...
def detect(bgr, w, tid, bary, triangles, vertices, w14):
    ...
    # effective resolution median from quality weight? we don't have er map here, estimate from focus proxy
    # Use valid pixel count as proxy for er if not provided via w? Better: assume 1.2 if unknown, but try to infer from w mean
    # w is quality_weight 0..1, not er. We can approximate er_median = 1.2 / (1 - w.mean + eps) ??? For now use 1.2 default
    # In pipeline.py we will soon pass er map — this detect() should accept optional kw but keep signature, so we detect er via extra global? For drop-in we use 1.2
    # However, if pipeline passes er via w14? No. So we keep 1.2 fallback, but pipeline_v4 will call response_map_scale_adaptive with er median from quality_maps
    er_median = 1.2
    # attempt to estimate er median from bary/triangle density? skip for now
    ...
    # endpoint/junction counts from degree map + graph
    endpoint_count = 0
    junction_count = 0
    if skan_graph is not None:
        try:
            # skan provides degree? approximate via path endpoints
            endpoint_count = len(branches) * 2  # each path 2 endpoints (rough)
            # junction: estimate via overlapping coordinates? For simplicity count degree>=3 pixels in degree map within skeleton
            junction_count = int(np.sum((degree>=4) & sk))  # degree map includes center, so 4 means 3 neighbors + center
        except:
            pass
