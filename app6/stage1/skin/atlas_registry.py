"""✅ VERIFIED → Реестр атласов: верификация топологии (sha) и валидация версий.
🚪 API: verify_topology(), validate(), describe()
🚨 WARNING: несовпадение sha атласа = hard fail — нельзя смешивать версии.
"""
...
class AtlasRegistry:
 ...
 # ✅ VERIFIED → сверка sha топологии атласа с реестром
 def verify_topology(self,triangles):
  ...
  return True
 # ✅ VERIFIED → валидация версии атласа (hard fail при mismatch)
 def validate(self):
 ...
 # 📤 Описание активного атласа для отчётов
