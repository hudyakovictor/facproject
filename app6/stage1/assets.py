...
def save_uv_and_mesh(bgr: np.ndarray, bundle: Any, out: Path, uv_size: int, skin_mask: np.ndarray | None = None) -> tuple[dict[str, str], dict[str, np.ndarray], dict[str, float]]:
    from uv_module import HDUVConfig, HDUVTextureGenerator
    from uv_module.forensics import extract_texture_forensics
    from uv_module.pose_analysis import analyze_pose_zones
    ...
    if not cv2.imwrite(str(out / "uv_analysis.png"), analysis):
        raise OSError("failed to write analytical UV texture")
    if not cv2.imwrite(str(out / "uv_synthetic.png"), synthetic):
        raise OSError("failed to write synthetic UV texture")
    if not cv2.imwrite(str(out / "uv_preview.png"), synthetic):
        raise OSError("failed to write preview UV texture")
    # Backward-compatible render texture: deliberately points to the visual-only product.
    if not cv2.imwrite(str(out / "uv_texture.png"), synthetic):
        raise OSError("failed to write compatibility UV texture")
    forensic_report, _ = extract_texture_forensics(analysis, np.asarray(observed, bool), out)
    wrinkle_report = analyze_pose_zones(
        texture_bgr=analysis,
        observed_mask=np.asarray(observed, bool),
        pose_bin=str(bundle.pose_bin),
        vertices_canonical=np.asarray(bundle.vertices_object_normalized, np.float32),
        vertices_2d=np.asarray(vertices_2d, np.float32),
    ...
    if not cv2.imwrite(str(out / "uv_confidence.png"), valid_mask.astype(np.uint8) * 255):
        raise OSError("failed to write UV confidence mask")
