...
РОЛИ (расширение v2, AUDIT-4 — стандартизированы по всему коду):
  🔢  NUMERIC       — Чистая вычислительная функция (матрицы/геометрия)
  🔍  QUERY         — Поиск/выборка без мутации состояния
  📤  OUTPUT        — Сериализация/вывод (json, csv, рисунок, строки отчёта)
  ⚙️  CONFIG        — Конфигурация/пороги/политики
  🚧  GATE          — Гейт/ограничитель применимости (может отклонять вход)
  🔀  MERGE         — Слияние структур (defaults + overrides)
  🔒  GUARD/FINAL   — Гвард состояния или запись о финализации (read-only)
  📜  CONTRACT      — Юридический/формальный контракт вывода
  📦  PACKAGE       — Маркер пакета (__init__.py)
  🗑️  DEPRECATED    — Устаревшее, оставлено для совместимости (см. §4)

СТАТУСЫ РАНТАЙМ-ЛОГА (status_logger.STATUS_FLOW, см. FUNCTION_STATUS_LOG.md):
  🔴  need_testing  — Работает без ошибок, требует проверки
  ✅  complete      — Проверено и работает корректно
  🚪  closed        — Протестировано и одобрено (скрыто из консоли)
  ⚠️  in_progress   — Частично реализовано
  🚫  blocked       — Заблокировано другой нереализованной функцией
  ❌  error         — Известный баг
  🔬  experimental  — Экспериментальное
  🗑️  deprecated    — Устаревшее
  ⏳  WAITING       — (в STATUS_AUDIT) Ожидает завершения другой функции
