...
export default function MeshViewer({
  points106, points134, heatmapPoints, fullMesh, morphT = 0, wireframe = true, showVectors = false, showPoints = true,
  ...
  useEffect(() => {
    ...
    if (fullMesh && fullMesh.vertices.length > 0) {
      ...
      if (showVectors && target) {
        ...
        fullMesh.vertices.forEach(([x, y, z], i) => {
          ...
          // Only show vectors for meaningful displacement to reduce visual noise
          if (dist > 0.002) {
            vectorPositions.push(x, y, z);
            vectorPositions.push(tx, ty, tz);
            // Color vector by magnitude
            const normalized = dist / maxRef;
            const c = colorFor(normalized);
            vectorColors.push(c.r, c.g, c.b);
            vectorColors.push(c.r, c.g, c.b);
          }
        });
        if (vectorPositions.length > 0) {
          const vGeo = new THREE.BufferGeometry();
          vGeo.setAttribute("position", new THREE.Float32BufferAttribute(vectorPositions, 3));
          vGeo.setAttribute("color", new THREE.Float32BufferAttribute(vectorColors, 3));
          const vMat = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.7 });
          state.group.add(new THREE.LineSegments(vGeo, vMat));
        }
      }
    ...
    return () => disposeGroupChildren(state.group);
  }, [points106, points134, heatmapPoints, fullMesh, morphT, wireframe, showVectors, showPoints, heatmapStops, gradient, knnLinePositions]);
