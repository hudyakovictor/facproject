...
const MINIMAP_H = 32;       // картографическая миникарта сверху
...
export default function UnifiedTimeline({
  ...
  const isTrackVisible = useCallback((id: string) => {
    if (soloTrack && soloTrack !== id) return false;
    if (mutedTracks.has(id)) return false;
    return true;
  }, [soloTrack, mutedTracks]);
  ...
  const yMinimap = 0;
  const yPins = yMinimap + MINIMAP_H;
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 bg-surface border-r border-border-strong" style={{ width: LABEL_W, height: totalContentH }}>
        {/* minimap label */}
        <div className="px-2 flex justify-between items-center font-mono text-[9px] text-[#5591c7] tracking-forensic border-b border-border bg-[#0a0a0a]"
          style={{ height: MINIMAP_H }}>
          <span className="flex items-center gap-1.5"><Icon name="search" size={11} /> MINIMAP</span>
          <span className="text-[#797876]">{((visibleCount / totalPhotos) * 100).toFixed(1)}%</span>
        </div>
        ...
        {GEOM_TRACKS.map(tr => (
          <div key={tr.id}
            className={`group relative flex items-center gap-2 px-2 border-b border-[#222] font-mono text-[10px] hover:bg-[#111] transition-opacity ${!isTrackVisible(tr.id) ? 'opacity-30' : ''}`}
            style={{ height: TRACK_H_GEOM }}>
            <div className="absolute left-0 top-0 bottom-0 flex items-center bg-gradient-to-r from-[#000] via-[#000] to-transparent px-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 gap-0.5 w-10">
              <button onClick={() => toggleSolo(tr.id)} className={`w-3.5 h-3.5 flex items-center justify-center text-[7px] border ${soloTrack === tr.id ? 'bg-[#5591c7]/20 border-[#5591c7] text-[#5591c7]' : 'border-[#333] text-[#797876] hover:text-white'}`}>S</button>
              <button onClick={() => toggleMute(tr.id)} className={`w-3.5 h-3.5 flex items-center justify-center text-[7px] border ${mutedTracks.has(tr.id) ? 'bg-[#ff3b30]/20 border-[#ff3b30] text-[#ff3b30]' : 'border-[#333] text-[#797876] hover:text-white'}`}>M</button>
            </div>
            <div className="w-1 h-4 ml-1 flex-shrink-0" style={{ background: tr.color }} />
        ...
        <div className={`group relative flex items-center gap-2 px-2 border-b border-[#222] font-mono text-[10px] hover:bg-[#111] transition-opacity ${!isTrackVisible(AGE_TRACK.id) ? 'opacity-30' : ''}`}
          style={{ height: TRACK_H_AGE }}>
          <div className="absolute left-0 top-0 bottom-0 flex items-center bg-gradient-to-r from-[#000] via-[#000] to-transparent px-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 gap-0.5 w-10">
            <button onClick={() => toggleSolo(AGE_TRACK.id)} className={`w-3.5 h-3.5 flex items-center justify-center text-[7px] border ${soloTrack === AGE_TRACK.id ? 'bg-[#5591c7]/20 border-[#5591c7] text-[#5591c7]' : 'border-[#333] text-[#797876] hover:text-white'}`}>S</button>
            <button onClick={() => toggleMute(AGE_TRACK.id)} className={`w-3.5 h-3.5 flex items-center justify-center text-[7px] border ${mutedTracks.has(AGE_TRACK.id) ? 'bg-[#ff3b30]/20 border-[#ff3b30] text-[#ff3b30]' : 'border-[#333] text-[#797876] hover:text-white'}`}>M</button>
          </div>
          <div className="w-1 h-7 ml-1 flex-shrink-0" style={{ background: AGE_TRACK.color }} />
          <div className="flex-1 min-w-0">
      ...
      >
        ...
        <g transform={`translate(0, 0)`}>
          <rect x={0} y={0} width={trackAreaW} height={MINIMAP_H} fill="#050505" />
          {/* Density plot or simple lines for all photos */}
          {(() => {
            if (!photos.length) return null;
            const tMin = photos[0].t;
            const tMax = photos[photos.length - 1].t;
            const tSpan = Math.max(1, tMax - tMin);
            const pts = photos.map(p => {
              const nx = (p.t - tMin) / tSpan;
              return { x: nx * trackAreaW, q: p.quality };
            });
            return (
              <g>
                {pts.map((p, i) => (
                  <line key={`mm-${i}`} x1={p.x} y1={MINIMAP_H - p.q * MINIMAP_H} x2={p.x} y2={MINIMAP_H} stroke="#4f98a3" strokeOpacity={0.4} strokeWidth={1} />
                ))}
              </g>
            );
          })()}
          ...
          {(() => {
            ...
            const v0 = photos[firstVisible]?.t ?? tMin;
            const v1 = photos[Math.min(photos.length - 1, lastVisible)]?.t ?? tMax;
            
            const x0 = ((v0 - tMin) / tSpan) * trackAreaW;
            const x1 = ((v1 - tMin) / tSpan) * trackAreaW;
            return (
              <g>
                <rect x={x0} y={0} width={Math.max(2, x1 - x0)} height={MINIMAP_H} fill="#5591c7" fillOpacity={0.2} stroke="#5591c7" strokeWidth={1} />
              </g>
            );
          })()}
        </g>
        ...
        {GEOM_TRACKS.map((tr, i) => {
          if (!isTrackVisible(tr.id)) return null;
        ...
        {(() => {
          if (!isTrackVisible(AGE_TRACK.id)) return null;
          ...
          const i0 = Math.max(0, offset - 1);
          const i1 = Math.min(photos.length, offset + visibleCount + 1);
          let visualD = "", calD = "", visualFill = "", corridorFill = "";
          const ptsV: { x: number; y: number; gi: number; v: number; cal: number; diff: number; yCorridorUpper: number; yCorridorLower: number }[] = [];
          for (let i = i0; i < i1; i++) {
            const p = photos[i];
            const x = xForIdx(i);
            const yV = yVal(p.visualAge);
            const yC = yVal(p.calendarAge);
            // Expected aging corridor ± 5 years
            const yCorridorUpper = yVal(p.calendarAge + 5);
            const yCorridorLower = yVal(p.calendarAge - 5);
            ptsV.push({ x, y: yV, gi: i, v: p.visualAge, cal: p.calendarAge, diff: p.visualAge - p.calendarAge, yCorridorUpper, yCorridorLower });
          ...
          if (ptsV.length) {
            ...
            corridorFill = `M ${ptsV[0].x.toFixed(1)} ${ptsV[0].yCorridorUpper.toFixed(1)} `
              + ptsV.map(p => `L ${p.x.toFixed(1)} ${p.yCorridorUpper.toFixed(1)}`).join(" ")
              + ` L ${ptsV[ptsV.length - 1].x.toFixed(1)} ${ptsV[ptsV.length - 1].yCorridorLower.toFixed(1)} `
              + [...ptsV].reverse().map(p => `L ${p.x.toFixed(1)} ${p.yCorridorLower.toFixed(1)}`).join(" ")
              + " Z";
          }
          return (
            <g>
              <path d={corridorFill} fill="#5591c7" fillOpacity="0.1" />
      ...
      <div className="absolute" style={{ left: LABEL_W, top: 0, width: trackAreaW, height: totalContentH, pointerEvents: "none" }}>
        ...
        <div className="absolute left-0 right-0 pointer-events-none" style={{ top: yFilm, height: FILMSTRIP_H }}>
          {visiblePhotos.map((p, i) => {
            const gi = firstVisible + i;
            const xLocal = xForIdxLocal(gi);
            const color = HYPOTHESIS_COLORS[p.dominant] ?? HYPOTHESIS_COLORS.UNAVAILABLE;
            const isSelected = selectedId === p.id;
            const isHovered = hoverIdx === gi;
            const isHighlighted = highlightIds?.has(p.id);
            const qColor = !Number.isFinite(p.quality) ? "#797876" : p.quality > 0.6 ? "#6daa45" : p.quality > 0.35 ? "#e8af34" : "#a13544";
            ...
            return (
              ...
              >
                <div className="w-full h-full relative overflow-hidden"
                  ...
                  <img src={photoImageUrl(p.id, "thumbnail")} alt={p.id} className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
        ...
        return (
          <div className="fixed z-50 bg-bg border border-border-strong rounded p-2 pointer-events-none shadow-2xl"
            ...
            <div className="w-full h-32 mb-2 relative overflow-hidden"
              ...
              <img src={photoImageUrl(p.id, "thumbnail")} alt="" className="absolute inset-0 w-full h-full object-contain" />
