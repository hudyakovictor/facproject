...
function Limitations({ data }: { data: RunSummary }) {
  ...
  const items: { label: string; value: string; isCrit?: boolean }[] = [];
  ...
  if (skipped && typeof skipped === "object") {
    for (const [reason, count] of Object.entries(skipped as Record<string, unknown>)) {
      items.push({ label: `${t.runSkippedPairs}: ${reason.replace(/_/g, " ")}`, value: String(count), isCrit: true });
    }
  }
  if (typeof missingQc === "number" && missingQc > 0) {
    items.push({ label: t.runMissingQc, value: String(missingQc), isCrit: true });
  }

  if (items.length === 0) {
    return (
      <div className="bg-[#0a0a0a] border border-[#333] p-3 font-mono text-[10px] text-[#797876]">
        {t.runNoLimitations}
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#141414] border-l-4 border-[#e8af34] p-4">
      <div className="font-mono text-[10px] tracking-forensic text-[#e8af34] mb-3 flex items-center gap-2 uppercase">
        <Icon name="alert-triangle" size={12} color="#e8af34" /> {t.runLimitationsTitle}
      </div>
      <div className="space-y-1">
        {items.map(item => (
          <div key={item.label} className="flex justify-between gap-3 font-mono text-[9px] border-b border-[#222] pb-1">
            <span className="text-[#e2e2e8]">{item.label}</span>
            {item.value && <span className={`${item.isCrit ? "text-[#ff3b30]" : "text-[#e8af34]"} tabular-nums`}>{item.value}</span>}
          </div>
        ))}
      </div>
    </div>
...
/** Hash Quartet and Schema Info */
function HashQuartet({ manifest }: { manifest: RunSummary }) {
  const rm = manifest.categories?.I?.summary?.run_manifest as Record<string, unknown> | undefined;
  const hCode = rm?.code_hash || "unknown";
  const hConfig = rm?.config_hash || "unknown";
  const hModel = rm?.model_hash || "unknown";
  const stage1 = manifest.categories?.I?.summary?.stage1_manifest_digest || "unknown";
  const schema = manifest.schema || "unknown";

  return (
    <div className="bg-[#0a0a0a] border border-[#333] p-4 flex flex-col mb-4">
      <div className="font-mono text-[10px] tracking-forensic mb-3 text-[#6daa45] uppercase flex items-center gap-2">
        <Icon name="database" size={12} color="#6daa45" /> Dataset & Provenance Hashes (Quartet)
      </div>
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 font-mono text-[9px]">
        <HashRow label="Code Hash" val={String(hCode)} />
        <HashRow label="Config Hash" val={String(hConfig)} />
        <HashRow label="Model Hash" val={String(hModel)} />
        <HashRow label="Stage 1 Dataset" val={String(stage1)} />
      </div>
      <div className="mt-3 pt-2 border-t border-[#222] font-mono text-[9px] text-[#797876] flex justify-between">
        <span>Schema Version: <span className="text-[#e2e2e8]">{schema}</span></span>
        <span className="uppercase tracking-forensic">Immutable Audit Trail</span>
      </div>
    </div>
  );
}

function HashRow({ label, val }: { label: string; val: string }) {
  const short = val.length > 24 ? val.substring(0, 12) + "…" + val.substring(val.length - 12) : val;
  return (
    <div className="flex justify-between border-b border-[#222] pb-1 group">
      <span className="text-[#797876]">{label}</span>
      <span className="text-[#e2e2e8] font-bold cursor-crosshair" title={val}>{short}</span>
    </div>
  );
}
