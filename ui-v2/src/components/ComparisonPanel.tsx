...
export default function ComparisonPanel({ rangeA, rangeB, onClose, onSetSide, activeSide }: Props) {
  ...
  const stage2Available = [...rangeA.photos, ...(rangeB?.photos ?? [])].some(
  ...
  if (!stage2Available) return (
    <div ref={dialogRef} data-no-pan role="region" aria-label={t.comparisonMode} tabIndex={-1}
      className="absolute left-0 right-0 bottom-0 z-40 bg-surface border-t-2 border-warning shadow-2xl outline-none p-4" style={{ height: 180 }}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 font-display tracking-forensic text-sm"><Icon name="compare" size={16} color="#e8af34" />{t.comparisonMode}</div>
        <button onClick={onClose} aria-label={t.closeLabel} className="w-7 h-7 flex items-center justify-center border border-border"><Icon name="x" size={14} /></button>
      </div>
      <div role="status" className="border border-warning/50 bg-warning/10 p-3 font-mono text-[10px] text-warning">
        Сравнение диапазонов требует измеренных результатов Stage 2. Сейчас доступны только реальные записи инвентаря Stage 1; метрики и вероятности не подставляются.
      </div>
    </div>
  );
          ...
          {a ? (
            <div className="space-y-1 font-mono text-[10px]">
              ...
              <div className="flex justify-between"><span className="text-text-muted">{t.pAll}</span>
                <span><span style={{ color: HYPOTHESIS_COLORS.H0 }}>{fmtPercent(a.p0, 0).replace("%", "")}</span>/<span style={{ color: HYPOTHESIS_COLORS.H1 }}>{fmtPercent(a.p1, 0).replace("%", "")}</span>/<span style={{ color: HYPOTHESIS_COLORS.H2 }}>{fmtPercent(a.p2, 0).replace("%", "")}</span>%</span>
              </div>
            </div>
              ...
              {metrics.map(m => {
                ...
                return (
                  <div key={m.k} className={`grid grid-cols-12 px-2 py-0.5 font-mono text-[10px] border-t border-border ${exceeded ? "bg-critical/15" : ""}`}>
                    ...
                    <div className="col-span-2 text-right">{fmt(va, 3)}</div>
                    <div className="col-span-2 text-right">{fmt(vb, 3)}</div>
                    <div className="col-span-2 text-right" style={{ color: exceeded ? "#ff3b30" : "#7a7a8a" }}>{Number.isFinite(delta) ? `${delta >= 0 ? "+" : ""}${fmt(delta, 3)}` : "—"}</div>
        ...
        <div className="flex-1 p-3">
          ...
          {b ? (
            <div className="space-y-1 font-mono text-[10px]">
              ...
              <div className="flex justify-between"><span className="text-text-muted">{t.pAll}</span>
                <span><span style={{ color: HYPOTHESIS_COLORS.H0 }}>{fmtPercent(b.p0, 0).replace("%", "")}</span>/<span style={{ color: HYPOTHESIS_COLORS.H1 }}>{fmtPercent(b.p1, 0).replace("%", "")}</span>/<span style={{ color: HYPOTHESIS_COLORS.H2 }}>{fmtPercent(b.p2, 0).replace("%", "")}</span>%</span>
              </div>
            </div>
          ) : (
