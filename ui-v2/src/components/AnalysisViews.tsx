...
const finiteValues = (rows: Photo[], pick: (photo: Photo) => number) => rows.map(pick).filter(Number.isFinite);
const mean = (rows: Photo[], pick: (photo: Photo) => number) => {
  const values = finiteValues(rows, pick);
...
const median = (rows: Photo[], pick: (photo: Photo) => number) => {
  const values = finiteValues(rows, pick).sort((a, b) => a - b);
  if (!values.length) return Number.NaN;
  const mid = Math.floor(values.length / 2);
  return values.length % 2 === 0 ? (values[mid - 1] + values[mid]) / 2 : values[mid];
};
const anomaly = (p: Photo) => ["SUSPICIOUS_TEXTURE", "GEOMETRIC_MISMATCH", "IDENTITY_ANOMALY", "TEMPORAL_IMPOSSIBILITY"].includes(p.fuzzy);
...
function Spark({ values, color = "#5591c7", height = 120 }: { values: number[]; color?: string; height?: number }) {
  ...
  if (!clean.length) return <div role="status" className="flex items-center justify-center font-mono text-[10px] text-text-muted border border-border" style={{ height }}>Нет измеренных значений</div>;
  const min = Math.min(...clean), max = Math.max(...clean), span = max - min || 1;
  const points = clean.map((v, i) => `${(i / Math.max(1, clean.length - 1)) * 100},${height - 8 - ((v - min) / span) * (height - 16)}`).join(" ");
...
export default function AnalysisViews({ kind, photos, selected, onSelect, onInspect, onCompare, chronoAnomalies = {} }: Props) {
  ...
  const stage2Available = photos.some(p => p.analysisStage !== "stage1_inventory" && p.measurementStatus !== "not_compared");
  const unavailable = (title: string) => <Shell title={title} sub=""><div role="status" className="max-w-2xl border border-warning/50 bg-warning/10 p-5 font-mono text-[11px] text-warning">Этот экран требует сравнительных результатов Stage 2. Сейчас загружен только реальный инвентарь Stage 1; демонстрационные метрики не подставляются.</div></Shell>;
  ...
  if (kind === "MATRIX" && !stage2Available) return unavailable(t.avMatrixTitle);
  if (kind === "COMPARE" && !stage2Available) return unavailable(t.avCompareTitle);
  if (kind === "DRIFT" && !stage2Available) return unavailable(t.avDriftTitle);
  if (kind === "METRICS" && !stage2Available) return unavailable(t.avMetricsTitle);

  if (kind === "MATRIX") return <Shell title={t.avMatrixTitle} sub={t.avMatrixSub}><div className="overflow-auto border border-border"><table className="w-full min-w-[980px] border-collapse font-mono text-[10px]"><thead><tr><th className="sticky left-0 bg-surface p-2 text-left">{t.avColEra}</th>{POSE_BUCKETS.map(p => <th key={p} className="bg-surface p-2 text-text-muted">{POSE_LABELS[p]}</th>)}</tr></thead><tbody>{eras.map(era => <tr key={era}><th className="sticky left-0 bg-surface p-2 text-left" style={{ color: ERA_META[era]?.color ?? "#797876" }}>{ERA_META[era]?.label ?? era}</th>{POSE_BUCKETS.map(pose => { const rows = photos.filter(p => p.era === era && p.bucket === pose); const score = mean(rows, p => p.boneScore); return <td key={pose} className="border border-border p-2 text-center" title={t.avPhotoCount(rows.length)}><div className="text-base">{rows.length ? fmt(score, 3) : "—"}</div><div className="text-text-faint">n={rows.length}</div></td>; })}</tr>)}</tbody></table></div></Shell>;
