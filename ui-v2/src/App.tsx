...
import { exportFixCapsule, loadTimeline, type DataMode, type AnalysisStage } from "./api";
...
import { writeUiLog } from "./uiLogger";
...
function AppInner() {
  ...
  const [analysisStage, setAnalysisStage] = useState<AnalysisStage>("unknown");
  ...
  const refreshTimeline = useCallback((signal?: AbortSignal) => {
    ...
    return loadTimeline(signal).then(result => {
      ...
      setAnalysisStage(result.analysisStage);
      ...
      setTimelineError("");
      writeUiLog("info", `Хронология загружена: ${result.photos.length} фото`, "refreshTimeline", { rejected: result.rejected.length });
    }).catch((err: unknown) => {
      if (signal?.aborted) return;
      ...
      writeUiLog("error", message, "refreshTimeline");
      setTimelineError(message); setDataMode("error"); setAnalysisStage("unknown"); setDataMessage(message); setPhotos([]); setEraMeta({}); setRejectedRows([]); setChronoAnomalies({});
    }).finally(() => {
  ...
  const filteredPhotos = useMemo(() => {
    return photos.filter(p => {
      ...
      // Stage 1 не имеет confidence: отсутствие Stage 2 не должно превращать
      // валидный инвентарь в пустой экран. Порог confidence применяется только
      // к реально рассчитанным сравнительным результатам.
      if (p.analysisStage !== "stage1_inventory" && confidenceFloor > 0 && (!Number.isFinite(p.confidence) || p.confidence < confidenceFloor)) return false;
      if (filters.hideLowQuality && (!Number.isFinite(p.quality) || p.quality < qualityFloor)) return false;
      if (thresholds.quality_min > 0 && (!Number.isFinite(p.quality) || p.quality < thresholds.quality_min)) return false;
  ...
  useEffect(() => {
    if (photos.length && !photos.some(p => p.t === playheadT)) setPlayheadT(photos[0].t);
  }, [photos, playheadT]);
  ...
  return (
    <BaselineContext.Provider value={baseline}>
      ...
      {analysisStage === "stage1_inventory" && !timelineError && (
        <div role="status" className="flex-shrink-0 bg-warning/10 border-b border-warning/40 px-3 py-1.5 font-mono text-[10px] text-warning">
          Загружены {photos.length} реальных кадров Stage 1. Фото, даты, ракурсы, качество и артефакты доступны; сравнительные метрики, вероятности и итоговый отчёт появятся только после Stage 2/3.
        </div>
      )}
...
function HeaderBar({ thumbSize, setThumbSize, filters, setFilters, onSources, onExport, onExportJson, onToggleTheme, theme, viewMode, setViewMode, stats, dataMode, dataMessage, lastRunLabel, onFixCapsule, onRefresh, refreshing, backendUp, leftPanelOpen, onToggleLeftPanel, onOpenFilters, onOpenSettings, onToggleLanguage, language, onShowMorphPlayer }: HeaderBarProps) {
  ...
  const navItems = [
    ...
    { v: "DATA", l: t.navData },
  ] satisfies { v: ViewMode; l: string }[];
  return (
    <div data-no-pan className="h-12 bg-surface border-b border-border-strong flex items-center px-3 gap-2 flex-shrink-0 min-w-0 relative z-30">
      <div className="flex items-center gap-2" title={t.appSubtitle}>
      ...
      <div className="font-mono text-[9px] text-text-muted whitespace-nowrap" title={`${t.artifact} v2.1.0 · ${t.photos}: ${stats.filtered}/${stats.total} · ${t.anomalies}: ${stats.anomalies} · ${t.lastRun}: ${lastRunLabel}`}>
        <span>{stats.filtered}/{stats.total}</span>
        <span className="mx-1 text-text-faint">·</span>
        <span className="text-critical">!{stats.anomalies}</span>
      </div>
      ...
      <div className="flex items-center gap-1 min-w-0 flex-1">
        <details className="relative flex-shrink-0"><summary className="list-none cursor-pointer px-2 py-1.5 border border-info/50 bg-info/10 text-text font-mono text-[10px] flex items-center gap-1 whitespace-nowrap"><Icon name="layers" size={12} /> {t.topMenuViews} <Icon name="chevron-down" size={10} /></summary><div className="absolute left-0 top-9 z-50 min-w-[220px] max-h-[70vh] overflow-auto bg-surface border border-border-strong shadow-2xl p-1"><div className="px-2 py-1 text-[9px] text-text-faint">{t.topMenuViews}</div>{navItems.map(item => <button key={item.v} onClick={() => setViewMode(item.v)} className={`w-full text-left px-2.5 py-2 font-mono text-[10px] hover:bg-surface-3 ${viewMode === item.v ? "text-info bg-info/10" : "text-text-muted"}`}>{item.l}</button>)}</div></details>
        <details className="relative flex-shrink-0"><summary className="list-none cursor-pointer px-2 py-1.5 border border-border bg-surface-2 text-text-muted font-mono text-[10px] flex items-center gap-1 whitespace-nowrap"><Icon name="database" size={12} /> {t.topMenuData} <Icon name="chevron-down" size={10} /></summary><div className="absolute left-0 top-9 z-50 min-w-[220px] bg-surface border border-border-strong shadow-2xl p-1"><div className="px-2 py-1 text-[9px] text-text-faint">{t.dataset}</div>{(["main", "calibration"] as const).map(d => <button key={d} onClick={() => setFilters(prev => ({ ...prev, dataset: d }))} className={`w-full text-left px-2.5 py-2 font-mono text-[10px] hover:bg-surface-3 ${filters.dataset === d ? "text-info" : "text-text-muted"}`}>{d === "main" ? t.datasetMain : t.datasetCalibration}</button>)}<button onClick={onSources} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.sources}</button><button onClick={() => setViewMode("DATA")} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.navData}</button></div></details>
        <details className="relative flex-shrink-0"><summary className="list-none cursor-pointer px-2 py-1.5 border border-border bg-surface-2 text-text-muted font-mono text-[10px] flex items-center gap-1 whitespace-nowrap"><Icon name="sliders" size={12} /> {t.topMenuTools} <Icon name="chevron-down" size={10} /></summary><div className="absolute left-0 top-9 z-50 min-w-[220px] bg-surface border border-border-strong shadow-2xl p-1"><button onClick={onOpenFilters} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.filters}</button><button onClick={onShowMorphPlayer} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.morphPlayer}</button><button onClick={onFixCapsule} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.fixCapsuleLabel}</button><button onClick={onOpenSettings} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.openSettings}</button></div></details>
        <details className="relative flex-shrink-0"><summary className="list-none cursor-pointer px-2 py-1.5 border border-border bg-surface-2 text-text-muted font-mono text-[10px] flex items-center gap-1 whitespace-nowrap"><Icon name="download" size={12} /> {t.topMenuExport} <Icon name="chevron-down" size={10} /></summary><div className="absolute left-0 top-9 z-50 min-w-[210px] bg-surface border border-border-strong shadow-2xl p-1"><button onClick={onExport} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text hover:bg-info/20">{t.exportReport}</button><button onClick={onExportJson} className="w-full text-left px-2.5 py-2 font-mono text-[10px] text-text-muted hover:bg-surface-3">{t.exportJson}</button></div></details>
      </div>
      <button onClick={onToggleLeftPanel} aria-label={t.detailPanel} className={`w-8 h-7 flex-shrink-0 flex items-center justify-center border ${leftPanelOpen ? "bg-info/20 border-info" : "border-border text-text-muted hover:text-text bg-surface-2"}`} title={t.detailPanel}><Icon name="panel-left" size={14} /></button>
      <select value={filters.bucketFilter} aria-label={t.bucket} onChange={e => setFilters(prev => ({ ...prev, bucketFilter: e.target.value === "all" ? "all" : e.target.value as PoseBucket }))} className="max-w-[128px] bg-surface-2 border border-border px-2 py-1.5 font-mono text-[10px] text-text flex-shrink-0"><option value="all">{t.bucket}: {t.bucketAll}</option>{POSE_BUCKETS.map(bucket => <option key={bucket} value={bucket}>{POSE_LABELS[bucket]}</option>)}</select>
      <div className="relative flex-shrink-0"><button onClick={() => setSearchOpen(v => !v)} aria-label={t.searchPlaceholder} className="w-8 h-8 flex items-center justify-center border border-border bg-surface-2 text-text-muted hover:text-text"><Icon name="search" size={13} /></button>{searchOpen && <input autoFocus value={filters.search} placeholder={t.searchPlaceholder} onChange={e => setFilters(prev => ({ ...prev, search: e.target.value }))} className="absolute right-0 top-9 z-50 w-56 bg-surface border border-info px-2 py-1.5 font-mono text-[10px] text-text" />}</div>
      <button onClick={onToggleLanguage} aria-label={t.languageToggle} className="px-2 py-1.5 font-mono text-[10px] border border-border bg-surface-2 text-text-muted flex-shrink-0">{language.toUpperCase()}</button>
      <button onClick={onToggleTheme} aria-label={t.toggleTheme} className="w-8 h-8 flex-shrink-0 flex items-center justify-center border border-border bg-surface-2 text-text-muted"><Icon name={theme === "dark" ? "sun" : "moon"} size={13} /></button>
      ...
      <button onClick={onRefresh} disabled={refreshing}
        aria-label={refreshing ? t.refreshing : t.refreshTimeline} title={t.refreshTimelineHint}
      ...
      <div title={`${dataMode === "research" ? t.dataModeResearch : dataMode === "loading" ? t.dataModeLoading : "ERROR"} · ${dataMessage} · ${t.notAVerdict}`} className={`px-1.5 py-1 border font-mono text-[9px] tracking-forensic ${dataMode === "research" ? "border-nominal/50 text-nominal" : dataMode === "loading" ? "border-info/50 text-info" : "border-warning/50 text-warning"}`}>{dataMode === "research" ? t.dataModeResearch : dataMode === "loading" ? t.dataModeLoading : t.dataModeError} · {t.notAVerdict}</div>

      {/* Размер миниатюр — единственный быстрый контрол, остальные действия
          находятся в компактных выпадающих группах и не дублируются. */}
