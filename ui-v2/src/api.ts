...
export type AnalysisStage = "stage1_inventory" | "stage2_analysis" | "stage3_report" | "unknown";

export interface TimelinePayload {
  schema?: string; source_mode?: string; analysis_stage?: string; photos?: unknown[]; items?: unknown[]; note?: string;
...
export interface TimelineResult {
  photos: Photo[]; mode: DataMode; message: string; analysisStage: AnalysisStage;
...
function validatePhoto(value: unknown, knownEras: ReadonlySet<string>): { ok: true; photo: Photo } | { ok: false; id: string; reason: string } {
  ...
  // Stage 1 — полноценный инвентарь кадров: сравнительные метрики ещё могут
  // отсутствовать, но сам кадр, дата, ракурс и качество должны отображаться.
  // Нельзя отбрасывать такую строку только из-за того, что Stage 2 ещё не запущен.
...
export async function loadTimeline(signal?: AbortSignal): Promise<TimelineResult> {
  ...
  writeUiLog("info", "Запрос хронологии", "loadTimeline", { endpoint });
  ...
  for (const row of rows) {
    ...
    if (verdict.ok) photos.push(verdict.photo);
      ...
      rejected.push({ id: verdict.id, reason: verdict.reason });
      writeUiLog("warn", `Строка отклонена: ${verdict.reason}`, "timeline.validation", { id: verdict.id });
    }
  }
  if (!photos.length) {
    const detail = rejected.length ? `; rejected ${rejected.length}: ${rejected[0].reason}` : "";
    writeUiLog("error", `Нет валидных фотографий${detail}`, "loadTimeline");
  ...
  const rawStage = Array.isArray(payload) ? "unknown" : payload.analysis_stage;
  const analysisStage: AnalysisStage = rawStage === "stage1_inventory" || rawStage === "stage2_analysis" || rawStage === "stage3_report"
    ? rawStage : "unknown";
  return {
    photos: sorted,
    mode: "research",
