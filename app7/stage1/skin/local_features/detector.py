"""Noise-anchored local skin feature detector."""
...
def detect(bgr, w, tid, bary, triangles, vertices, max_candidates=500):
    """Detect local anomalous features (pores, spots, scars) via noise-anchored thresholding."""
    ...
    for i in order[:max_candidates]:
        ...
        x, y = map(int, np.round(cents[i]))
        x = np.clip(x, 0, r.shape[1] - 1)
        y = np.clip(y, 0, r.shape[0] - 1)
        fi = int(tid[y, x])
        if area < 3 or area > 500 or fi < 0:
            continue
        ...
        contrast = (float(L[region].mean()) - float(L[ring].mean())) / max(float(L[ring].mean()), 0.05) if ring.any() else np.nan
        rows.append((len(rows), x, y, fi, *bc, *pos, area, ecc, contrast, float(r[region].max() / noise_sigma)))
    dt = np.dtype([
        ("candidate_id", "i4"), ("x", "i4"), ("y", "i4"), ("triangle_id", "i4"),
