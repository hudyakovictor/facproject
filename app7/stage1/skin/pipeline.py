"""Skin evidence pipeline — single-photo extraction.

Key design decisions (vs app6):
1. FFHQ adapter receives ORIGINAL crop (not masked) — it has its own parser
2. Photometric normalization applied before texture feature extraction
3. Preview count controlled by preview_level: none|minimal|full
4. No SHA-256 on source photos
5. Material evidence is experimental indicators only (no verdict)
6. All output is within skin/ subdirectory of the photo directory
"""
...
def _resolve_pose_policy_csv(atlas_path: Path) -> Path:
    ...
    for c in candidates:
        if c.is_file():
            return c
    raise FileNotFoundError("pose_policy_v3_9bins.csv not found: " + ", ".join(str(c) for c in candidates))


def build_skin_package(
    *, photo_id, input_path, bgr, out_dir, triangles, vertices_original_xy,
    vertices_depth, normals, surface_vertices, vertex_visibility,
    face_mask_data_path, atlas_path, coordinate_chain, models, config, pose,
    preview_level="minimal",
):
    """Build complete skin evidence package for one photo.

    Args:
        bgr: Original full-resolution BGR image
        face_mask_data_path: Path to face_mask.npz with mask_original
        atlas_path: Path to texture_zones_bfm35709_v3.npz
        preview_level: "none" | "minimal" | "full"
    """
    # ── 1. Load mask ──
    face_mask_data_path = Path(face_mask_data_path)
    if not face_mask_data_path.is_file():
        raise ValueError("face_mask.npz unavailable")
    ...
    # ── 2. Atlas + manifest ──
    atlas = AtlasRegistry.load(atlas_path, triangles)
    manifest = create_manifest(
        photo_id, input_path, bgr,
        coordinate_chain=coordinate_chain, models=models, atlas=atlas.describe(),
        config=config, backend={"rasterizer": "numpy_cpu_zbuffer_v2_physics_density"},
        warnings=["app7 v1.0: FFHQ receives original crop; photometric normalization for texture"],
    )

    # ── 3. Crop to face bounding box ──
    H, W = bgr.shape[:2]
    ...
    # ── 4. Contamination (hair, glasses, occlusions) ──
    ...
    if fp.is_file():
        ...
        atomic_npz(root / "contamination_maps.npz",
                    hair=cont["hair"], glasses=cont["glasses"], external_occlusion=cont["external_occlusion"])
        contamination_meta = {"state": "complete", **parser.metadata()}

    # ── 5. Surface rasterization + atlas projection ──
    try:
    ...
    # ── 7. Quality maps ──
    qm = quality_maps(crop, domain, r.incidence, r.projection_confidence, r.triangle_id,
    ...
    # ── 8. Pose policy (soft evidence) ──
    policy_csv = _resolve_pose_policy_csv(atlas_path)
    ...
    soft_pose, soft_meta, observed = policy.soft_evidence_weights(
        ...
        domain=domain, projection_confidence=r.projection_confidence,
        incidence=r.incidence, visibility=r.visibility,
    )
    pm.update(soft_meta)
    qm["pose_weight"] = pose_prior.astype(np.float32)
    qm["pose_soft_weight"] = soft_pose.astype(np.float32)
    ...
    zone_app = per_zone_applicability(p["zone_id_a20"], domain, qm["quality_weight"], pose_weight=pose_prior)
    ...
    # ── 10. Save structured data ──
    surface_area = tri_area if tri_area is not None else np.ones(len(triangles), np.float32)
    atomic_npz(root / "surface_observations.npz",
                schema=np.array(SCHEMAS["surface"]),
                triangle_id=r.triangle_id, barycentric=r.barycentric.astype(np.float16),
                source_xy=r.source_xy, depth=r.depth,
                normal=r.normal.astype(np.float16), incidence=r.incidence.astype(np.float16),
                ...
                map_origin_xy=np.array([x0, y0]), original_shape=np.array([H, W]),
    ...
    pd_type = np.dtype([("level", "U3"), ("zone", "i2"), ("patch_id", "U24"),
                         ("x0", "i4"), ("y0", "i4"), ("x1", "i4"), ("y1", "i4"),
                         ("pixels", "i4"), ("support", "f4")])
    if patches:
        atomic_npz(root / "patch_index.npz", schema=np.array("skin-patch-index-v1"),
                    patches=np.array(patches, dtype=pd_type))

    # ── 13. Basic macro features ──
    basic = extract_basic(crop, qm["quality_weight"], p["zone_id_a20"], p["zone_id_s40"])
    atomic_npz(root / "features/basic_macro.npz",
    ...
    # ── 14. Texture features (with photometric normalization) ──
    ...
    if ap["pigmentation"]["state"] not in {"usable", "coarse_only"}:
        for row in texture:
            row["values"][21:24] = np.nan
    atomic_npz(root / "features/texture.npz",
                schema=np.array(SCHEMAS["features"]),
    ...
    atomic_npz(root / "features/local_candidates.npz", schema=np.array(SCHEMAS["features"]),
                response=lr.astype(np.float16), candidates=lc)
    atomic_json(root / "features/local_candidates_meta.json", {"metadata": lm})

    # ── 16. Wrinkles: classical (Frangi/Meijering/Skan) ──
    ...
    atomic_npz(root / "wrinkles/classical.npz", schema=np.array(SCHEMAS["wrinkles"]),
                ridge_probability=ridge.astype(np.float16), skeleton=sk, points=points)

    # ── 17. Wrinkles: FFHQ (receives ORIGINAL crop, not masked!) ──
    ffstate = "not_run_weights_unavailable"
    ffmeta = {}
    prob = None
    cpdir = Path(atlas_path).resolve().parents[2] / "FFHQ-detect-face-wrinkles/res/cp"
    cp = next((x for x in (cpdir / "wrinkle_model.pth", cpdir / "best_checkpoint_iou032.pth") if x.is_file()),
              cpdir / "wrinkle_model.pth")
    if cp.is_file():
        ad = FFHQWrinkleAdapter(cp.parents[2], cp)
        # *** KEY FIX: pass original crop, not masked ***
        prob = ad.predict(crop)
        # Zero outside usable evidence mask
        prob[~usable_mask] = 0
        atomic_npz(root / "wrinkles/ffhq.npz", schema=np.array(SCHEMAS["wrinkles"]),
                    probability=prob.astype(np.float16))
        ffstate = "complete"
        ffmeta = ad.metadata()
    ...
    # ── 21. Spatial skin signatures ──
    try:
        from .spatial_signatures import compute_zone_spatial_signatures
        spatial_sig = compute_zone_spatial_signatures(
            crop, p["zone_id_a20"], qm["quality_weight"],
            ridge_probability=np.asarray(ridge, np.float32) if ridge is not None else None,
            ffhq_probability=prob if ffstate == "complete" else None,
            domain_mask=domain,
        )
        atomic_json(root / "spatial_signatures.json", spatial_sig)
    except Exception:
        pass

    # ── 20. Previews ──
    save_previews(root / "previews", crop, p["zone_id_a20"], domain, qm["quality_weight"],
                   usable_mask=usable_mask, level=preview_level)
    save_wrinkle_overlay(root / "previews", crop, sk, ridge, prob if ffstate == "complete" else None,
                          domain, usable_mask=usable_mask, level=preview_level)
