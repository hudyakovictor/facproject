"""Geometric utilities — pose classification, mesh normalization, coordinate transforms."""
...
def classify_pose(yaw: float) -> tuple[str, float]:
    """Classify yaw angle into a named pose bin with canonical yaw."""
...
def row_rotation_matrix(pitch_deg: float, yaw_deg: float, roll_deg: float) -> np.ndarray:
    """Build rotation matrix from Euler angles (row-major, Rz @ Ry @ Rx)."""
...
def normalize_mesh(mesh: np.ndarray) -> tuple[np.ndarray, np.ndarray, float]:
    """RMS-normalize mesh: center → divide by root-mean-square distance."""
...
def to_original_image(points_224: np.ndarray, trans_params: np.ndarray) -> np.ndarray:
    """Map 3DDFA image-plane coordinates (224×224) back to original image coordinates."""
...
def reprojection_stats(projected: np.ndarray, expected: np.ndarray) -> dict[str, float]:
    """Compute RMSE, P95, and max of per-point distances."""
    a = np.asarray(projected, np.float64)
