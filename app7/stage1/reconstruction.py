"""3DDFA_V3 reconstruction wrapper — single inference per photo."""
...
class ReconstructionEngine:
    """One network inference per photo; captures renderer output without patching."""

    def __init__(self, project_root: Path, device: str = "auto",
                 detector: str = "retinaface", backbone: str = "resnet50"):
        ...
        try:
            import sys
            d = str(self.project_root / "3ddfa_v3")
    ...
    def process(self, path: Path, oriented_rgb: np.ndarray | None = None) -> ReconstructionBundle:
        ...
        alpha_hook = self.model.net_recon.register_forward_hook(
            lambda _m, _i, o: captured_alpha.update({"alpha": o}))
        original_renderer_forward = self.model.renderer.forward

        def renderer_forward(*args, **kwargs):
        ...
        if alpha_t is None:
            raise RuntimeError("failed to capture net_recon output")
        ...
        expected106 = self._np(results["ldm106"])[0].astype(np.float32)
        expected134 = self._np(results["ldm134"])[0].astype(np.float32)

        seg = np.asarray(results.get("seg_visible"))
        while seg.ndim > 3:
            seg = seg[0]
        if seg.ndim == 3 and seg.shape[0] == 8:
        ...
        return ReconstructionBundle(
            ...
            semantic_channels_224=seg.astype(np.float16),
