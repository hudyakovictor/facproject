...
class visualize:
    ...
    def visualize_and_output(self, trans_params, img, save_path, img_name):
        ...
        if self.args.extractTex:
            ...
            # Save UV texture images if available
            if 'extractTex_uv' in self.result_dict:
                cv2.imwrite(os.path.join(save_path, img_name + '_extractTex_uv.png'), 
                           (self.result_dict['extractTex_uv']*255).astype(np.uint8)[:,:,::-1])
            if 'extractTex_uv_analytical' in self.result_dict:
                cv2.imwrite(os.path.join(save_path, img_name + '_extractTex_uv_analytical.png'), 
                           (self.result_dict['extractTex_uv_analytical']*255).astype(np.uint8)[:,:,::-1])
            if 'extractTex_uv_synthetic' in self.result_dict:
                cv2.imwrite(os.path.join(save_path, img_name + '_extractTex_uv_synthetic.png'), 
                           (self.result_dict['extractTex_uv_synthetic']*255).astype(np.uint8)[:,:,::-1])
