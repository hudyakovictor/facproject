...
def back_resize_ldms(ldms, trans_params):
    ldms = ldms.copy()  # avoid mutating caller's array
...
def back_resize_crop_img(img, trans_params, ori_img, resample_method = RESAMPLING_METHOD):
...
def resize_n_crop_img(img, lm, t, s, target_size=224., mask=None):
    ...
    w = int(w0*s)
    h = int(h0*s)
    t0 = float(t[0].item()); t1 = float(t[1].item())
...
def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    ...
    trans_params = np.array([w0, h0, s, float(t[0].item()), float(t[1].item())])
...
def load_lm3d():
    ...
    mat_path = os.path.join(assets_dir, "similarity_Lm3D_all.mat")
