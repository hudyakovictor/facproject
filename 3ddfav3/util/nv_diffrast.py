...
class MeshRenderer(nn.Module):
    ...
    def forward(self, vertex, tri, feat = None, visible_vertice = False):
        ...
        # Invert Y: 3DDFA uses bottom-origin; image space uses top-origin.
        # Clone first to avoid mutating the caller's tensor.
        vertex = vertex.clone()
        vertex[..., 1] = -vertex[..., 1] 
...
class MeshRenderer_UV(nn.Module):
    ...
    def forward(self, vertex, tri, feat = None, visible_vertice = False):
        ...
        if vertex.shape[-1] == 3:
            vertex = torch.cat([vertex, torch.ones([*vertex.shape[:2], 1]).to(device)], dim=-1)
        # Invert Y: clone first to avoid mutating the caller's tensor.
        vertex = vertex.clone()
        vertex[..., 1] = -vertex[..., 1] 
