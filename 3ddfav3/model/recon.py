...
# UVTextureGenerator imported lazily inside __init__ when extractTex=True
# to avoid crashing when the deprecated module is absent.
...
def process_uv(uv_coords, uv_h = 224, uv_w = 224):
    uv_coords = uv_coords.copy()  # avoid mutating the caller's array
...
class face_model:
    def __init__(self, args):
        ...
        model_path = os.path.join(assets_dir, "face_model.npy")
        ...
        if args.extractTex:
            # Lazy import: uv_texture_generator is DEPRECATED and may be absent.
            from util.uv_texture_generator import UVTextureGenerator
            # Initialize improved UV texture generator
            texture_size = getattr(args, 'texture_size', 1000)
            self.uv_texture_generator = UVTextureGenerator(
                uv_coords=model['uv_coords'].copy(),
                tri=self.tri.cpu().numpy(),
                device=self.device,
                texture_size=texture_size
            )
            # Keep old renderer for backward compatibility
            uv_coords_numpy = process_uv(model['uv_coords'].copy(), texture_size, texture_size)
            self.uv_coords_torch = (torch.tensor(uv_coords_numpy, requires_grad=False, dtype=torch.float32, device=self.device) / (texture_size - 1) - 0.5) * 2
            if self.device == 'cpu':
                from util.cpu_renderer import MeshRenderer_UV_cpu
                self.uv_renderer = MeshRenderer_UV_cpu(
                        rasterize_size=int(texture_size)
                )
            ...
            self.uv_coords_numpy[:,1] = texture_size - self.uv_coords_numpy[:,1] - 1
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load(os.path.join(assets_dir, "net_recon.pth"), map_location=torch.device('cpu'))['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load(os.path.join(assets_dir, "net_recon_mbnet.pth"), map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
    ...
    def forward(self, no_pca = False):
        ...
        if self.args.extractTex:
            # Use new texture generator if specified
            texture_mode = getattr(self.args, 'texture_mode', 'analytical')
            use_legacy = getattr(self.args, 'use_legacy_texture', False)
            ...
            if use_legacy:
                ...
                # New improved texture generation
                if texture_mode == 'analytical':
                    uv_texture, v_colors = self.uv_texture_generator.generate_analytical_texture(
                        face_texture, self.input_img, v2d, visible_idx
                    )
                    result_dict['extractTex_uv'] = uv_texture
                elif texture_mode == 'synthetic':
                    uv_texture, v_colors = self.uv_texture_generator.generate_synthetic_texture(
                        face_texture, self.input_img, v2d, visible_idx
                    )
                    result_dict['extractTex_uv'] = uv_texture
                elif texture_mode == 'both':
                    # Generate both versions
                    uv_texture_analytical, v_colors_analytical = self.uv_texture_generator.generate_analytical_texture(
                        face_texture, self.input_img, v2d, visible_idx
                    )
                    uv_texture_synthetic, v_colors_synthetic = self.uv_texture_generator.generate_synthetic_texture(
                        face_texture, self.input_img, v2d, visible_idx
                    )
                    result_dict['extractTex_uv_analytical'] = uv_texture_analytical
                    result_dict['extractTex_uv_synthetic'] = uv_texture_synthetic
                    v_colors = v_colors_analytical  # Use analytical as default for OBJ
                else:
