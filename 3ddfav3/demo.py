...
if __name__ == '__main__':
    ...
    parser.add_argument('--texture_mode', default='analytical', type=str,
                        choices=['analytical', 'synthetic', 'both'],
                        help='texture generation mode: analytical (enhanced quality), synthetic (mirrored for morphing), both (generate both)')
    parser.add_argument('--texture_size', default=1024, type=int,
                        help='UV texture resolution (default 1024, max 1024 for 800x800px input)')
    parser.add_argument('--use_legacy_texture', default=False, type=lambda x: x.lower() in ['true', '1'],
                        help='use legacy texture extraction instead of improved generator')
