...
def no_crop(im):
    if np.array(im).shape==(224,224,3):
        ...
        raise ValueError('the original face image should be well cropped and resized to (224,224,3).')

class retinaface:
    def __init__(self, device):
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        assets_dir = os.path.join(base_dir, "assets")
        ckpt_path = os.path.join(assets_dir, "large_base_net.pth")
        self.landmark_model = LargeModelInfer(ckpt_path, device=device)
    ...
    def detector(self, im):
        ...
        if len(results_all)>0:
            ...
            if np.array(im).shape==(224,224,3):
                ...
                raise RuntimeError('no face detected and image is not (224,224,3). Cannot continue.')
                raise RuntimeError('cannot continue without face detection')
