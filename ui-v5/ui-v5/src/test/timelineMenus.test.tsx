...
async function mount() {
  // Меню теперь живут в единой верхней панели, а не дублируются внутри страницы.
  const view = render(wrap(<><TopBar /><TimelinePage /></>));
  await waitFor(
    () => {
...
describe("меню «Ракурс» (§8.1)", () => {
  test("показывает количество, медиану качества и поддержку калибровки", async () => {
    ...
    const row = within(menu).getByRole("button", { name: /Фронтальный14/ });
    // Верхняя панель показывает фактическое число кадров в каждом бине.
  ...
  test("пустой бин нельзя выбрать, но он остаётся видимым", async () => {
    ...
    const empty = within(menu).getByRole("button", { name: /Левый профиль0/ });
  ...
  test("медиана качества не подменяется нулём при отсутствии оценок", async () => {
    ...
    const menu = screen.getByRole("group", { name: "Выбор ракурса" });
    const row = within(menu).getByRole("button", { name: /Левый профиль0/ });
...
describe("меню «Метрики» (§8.1)", () => {
  test("метрика без данных отключена и помечена", async () => {
    ...
    // Недоступная дорожка явно показывает нулевую доступность и отключена.
    expect(within(menu).getAllByText("0/40").length).toBeGreaterThan(0);
  ...
  test("калиброванные и диагностические величины различаются", async () => {
    ...
    const menu = screen.getByRole("group", { name: "Выбор метрик" });
    expect(within(menu).getAllByText("40/40").length).toBeGreaterThan(0);
    expect(within(menu).getAllByText("0/40").length).toBeGreaterThan(0);
  }, 30_000);
