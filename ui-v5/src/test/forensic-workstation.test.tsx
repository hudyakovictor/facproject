...
describe("v5 real-data UI contract", () => {
  test("timeline renders API-backed observation", async () => {
    renderWithApi(<TimelinePage />);
    await waitFor(() => expect(screen.getByTitle(/photo_001/)).toBeInTheDocument());
    expect(screen.getByTitle(/photo_001/)).toHaveAttribute("title", expect.stringContaining("2020-01-01"));
  });
