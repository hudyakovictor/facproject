...
const CountList: React.FC<{ counts?: Record<string, number> | null }> = ({ counts }) => {
  ...
  if (entries.length === 0) return <span className="text-slate-500">н/д</span>;
  return (
    <ul className="mt-1 flex flex-wrap gap-1.5">
      {entries.map(([key, value]) => (
        <li key={key} className="rounded bg-[#101820] px-2 py-0.5 font-mono text-[11px]">
          <span className="text-slate-500">{key}: </span>
          <span className="text-slate-200">{value.toLocaleString("ru-RU")}</span>
        </li>
      ))}
    </ul>
  );
