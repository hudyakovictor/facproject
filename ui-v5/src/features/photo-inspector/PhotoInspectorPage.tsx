...
export const PhotoInspectorPage: React.FC = () => {
  ...
  return (
    ...
    >
      {() => selected && (<div className="flex flex-col h-[calc(100vh-49px)] w-full bg-[#080d12] text-[#e2e8f0] overflow-y-auto p-6 space-y-5">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <section className="rounded-lg border border-[#1f2d3d] bg-[#0b1117] p-4"><div className="flex justify-between mb-3 font-mono text-xs text-slate-300"><span>ИСХОДНЫЙ КАДР STAGE 2</span><select aria-label="Выбор фотографии" value={selected.id} onChange={(e) => setSelectedId(e.target.value)} className="bg-[#101820] border border-[#1f2d3d] rounded px-2 py-1 max-w-[60%]">{photos.slice(0, 500).map((p) => <option key={p.id} value={p.id}>{p.date ?? p.id} · {p.id}</option>)}</select></div><div className="h-[380px] bg-[#101820] rounded border border-[#1f2d3d] flex items-center justify-center"><PhotoImage photoId={selected.id} alt={`Кадр ${selected.id}`} variant="contain" className="max-h-full max-w-full" /></div></section>
      <section className="rounded-lg border border-[#1f2d3d] bg-[#0b1117] p-4"><div className="mb-3 font-mono text-xs text-cyan-300">ПРЕВЬЮ РЕАЛЬНОГО КАДРА</div><div className="h-[380px] rounded border border-[#1f2d3d] bg-[#101820] flex items-center justify-center"><PhotoImage photoId={selected.id} alt={`Реальный кадр ${selected.id}`} variant="contain" className="max-h-full max-w-full" /></div><p className="mt-2 text-xs text-amber-300">Mesh-артефакт отсутствует в API. Синтетическая 3D-модель не показывается.</p></section>
    </div>
