...
export const DATA_COLUMNS: readonly DataColumn[] = [
  ...
  },
  {
    ...
    width: 130,
    origin: "absent",
    note: "API не отдаёт хеши файлов. Требуется задача B-02 (sidecar и целостность). До неё сверка целостности в интерфейсе невозможна.",
    value: () => null,
    render: () => "н/д",
    default: false,
  },
  {
    ...
    width: 120,
    origin: "absent",
    note: "Точные и перцептивные дубликаты вычисляются на стороне backend (B-02). Показывать пустое значение как «дубликатов нет» нельзя.",
    value: () => null,
    render: () => "н/д",
    default: false,
  },
