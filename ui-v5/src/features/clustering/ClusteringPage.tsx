    ...
    <div className="flex flex-col h-workspace w-full bg-surface-canvas text-ink-primary overflow-y-auto p-6 space-y-6">
      <div className="flex items-center justify-between rounded-lg border border-line-default bg-surface-base p-4">
        <div>
          ...
          <div className="text-xs text-ink-muted">Реальные наблюдения · {photos.length.toLocaleString("ru-RU")} фото · отображаются измерения, не вердикт</div>
        </div>
      ...
      <div className="rounded-lg border border-line-default bg-surface-base p-6 space-y-5">
        ...
        {groups.map(([bucket, bucketPhotos], index) => (
          <div key={bucket} className="space-y-1">
            ...
            <div className="relative h-9 w-full rounded bg-surface-raised border border-line-default">
      ...
      <div className="rounded-lg border border-line-default bg-surface-base p-5 space-y-3">
        ...
        <p className="text-xs text-ink-muted">API сообщает число change-point событий, но не отдаёт в текущем контракте дат, кластеров, ΔSNR или confidence для карточек. Поэтому эти значения не подменяются демонстрационными.</p>
