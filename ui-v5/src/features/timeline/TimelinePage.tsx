...
import { Activity, Check, ExternalLink, Filter, RefreshCw, Search, X } from "lucide-react";
import { researchTimeline, type ResearchPhoto } from "../../shared/researchApi";

const POSE_LABELS: Record<string, string> = {
  frontal: "Фронт", left_light: "Лево · лёгкий", left_mid: "Лево · средний", left_deep: "Лево · глубокий",
  left_profile: "Лево · профиль", right_light: "Право · лёгкий", right_mid: "Право · средний",
  right_deep: "Право · глубокий", right_profile: "Право · профиль",
};
const clamp = (v: number, min: number, max: number) => Math.max(min, Math.min(max, v));
const pct = (v: number | null) => v == null ? "—" : `${Math.round(v * 100)}%`;
const metric = (v: number | null) => v == null ? "—" : v.toFixed(1);

function isFinding(photo: ResearchPhoto) {
  return photo.flags.length > 0 || Object.entries(photo.stage2StatusCounts).some(([key, value]) => value > 0 && ["coherent_jump_candidate", "persistent_geometric_change", "insufficient_calibration"].includes(key));
}
function photoTone(photo: ResearchPhoto) {
  if (photo.flags.length) return "border-rose-600 bg-rose-950/60";
...
export function TimelinePage({ activePose = "frontal", qualityThreshold = 0 }: { activePose?: string; qualityThreshold?: number; mouthThreshold?: number }) {
  ...
  const firstDate = photos[0]?.t ?? Date.parse("1998-01-01");
  const lastDate = photos.at(-1)?.t ?? Date.parse("2026-05-13");
  const dateRange = Math.max(lastDate - firstDate, 1);
  const x = (photo: ResearchPhoto) => clamp(((photo.t ?? firstDate) - firstDate) / dateRange, 0, 1);
  ...
  const selectPhoto = (photo: ResearchPhoto) => {
    ...
    if (!aId) setAId(photo.id);
    else if (aId !== photo.id) setBId(photo.id);
  };
  ...
  if (query.isLoading) return <div className="p-6 font-mono text-sm text-cyan-300">Загрузка реального таймлайна Stage 2…</div>;
  if (query.error) return <div className="m-6 rounded border border-rose-800 bg-rose-950/40 p-4 font-mono text-sm text-rose-300">Не удалось загрузить timeline: {String(query.error)}</div>;
  ...
  return <main className="min-h-[calc(100vh-49px)] w-full overflow-auto bg-[#080d12] text-[#e2e8f0]">
    <section className="min-w-[980px] border-b border-[#263747] bg-[#0b1117]">
      <header className="flex items-center justify-between border-b border-[#263747] px-4 py-3"><div className="flex items-center gap-2"><Activity className="h-4 w-4 text-cyan-300" /><div><div className="font-mono text-sm font-bold text-cyan-300">ТАЙМЛАЙН · STAGE 2 · РЕАЛЬНЫЕ ДАННЫЕ</div><div className="mt-1 text-[11px] text-slate-500">{photos.length.toLocaleString("ru-RU")} фото · {years[0]}—{years.at(-1)} · один активный ракурс: {POSE_LABELS[activePose] ?? activePose}</div></div></div><div className="flex items-center gap-2"><button onClick={() => setFindingMode((v) => !v)} className={`rounded border px-3 py-1.5 text-xs ${findingMode ? "border-rose-600 bg-rose-950/50 text-rose-200" : "border-slate-700 text-slate-300"}`}><Filter className="mr-1 inline h-3.5 w-3.5" />Находки · {photos.filter(isFinding).length}</button><button onClick={() => void query.refetch()} title="Обновить" className="rounded border border-slate-700 p-2 text-slate-300"><RefreshCw className="h-4 w-4" /></button></div></header>
      ...
      <div ref={viewportRef} onWheel={handleWheel} className="overflow-x-auto overscroll-contain" style={{ scrollbarColor: "#155e75 #0a1016" }}>
        <div style={{ minWidth: contentWidth }}>
          <div className="grid grid-cols-[138px_1fr] border-b border-[#1c2935]"><div className="flex h-10 items-center border-r border-[#263747] bg-[#0a1016] px-3 font-mono text-[10px] text-slate-500">ЭПОХИ / ГОДЫ</div><div className="relative h-10">{years.map((year) => { const ratio = clamp((Date.parse(`${year}-01-01`) - firstDate) / dateRange, 0, 1); return <span key={year} className="absolute top-1.5 -translate-x-1/2 font-mono text-[10px] text-slate-500" style={{ left: `${ratio * 100}%` }}>{year}</span>; })}</div></div>
          {TRACKS.map(track)}
      ...
      <footer className="grid gap-2 border-t border-[#263747] px-4 py-2 text-[10px] text-slate-500 md:grid-cols-4"><span>После фильтров: {filtered.length.toLocaleString("ru-RU")}</span><span>Yaw: {photos.filter((p) => p.yaw != null).length} · Quality: {photos.filter((p) => p.quality != null).length}</span><span>Pairs: {photos.reduce((n, p) => n + p.stage2PairCount, 0).toLocaleString("ru-RU")}</span><span className="text-amber-300">Недоступные поля не подменяются: boneScore/p0–p2</span></footer>
    </section>
    {selectedPhoto && <aside className="fixed right-4 top-20 z-30 w-[min(430px,calc(100vw-2rem))] rounded-lg border border-cyan-700 bg-[#0b1117] p-4 shadow-2xl"><div className="flex items-start justify-between"><div><div className="font-mono text-xs font-bold text-cyan-300">PHOTO INSPECTOR · {selectedPhoto.id}</div><div className="mt-1 text-xs text-slate-400">{selectedPhoto.date ?? "дата отсутствует"} · {POSE_LABELS[selectedPhoto.bucket] ?? selectedPhoto.bucket}</div></div><button onClick={() => setSelected(null)}><X className="h-4 w-4 text-slate-400" /></button></div><img className="mt-3 max-h-44 w-full rounded border border-slate-800 object-contain" src={`/api/v1/photos/${encodeURIComponent(selectedPhoto.id)}/image`} alt={`Фото ${selectedPhoto.id}`} /><div className="mt-4 grid grid-cols-2 gap-2 font-mono text-[11px]">{[["quality", pct(selectedPhoto.quality)], ["yaw", metric(selectedPhoto.yaw)], ["pitch", metric(selectedPhoto.pitch)], ["roll", metric(selectedPhoto.roll)], ["evidence", selectedPhoto.evidenceState], ["measurement", selectedPhoto.measurementStatus], ["pairs", selectedPhoto.stage2PairCount], ["provenance", selectedPhoto.dateProvenanceStatus]].map(([key, value]) => <div key={key} className="rounded border border-slate-800 p-2"><div className="text-slate-500">{key}</div><div className="mt-1 text-slate-200">{String(value ?? "—")}</div></div>)}</div><div className="mt-3 rounded border border-slate-800 p-2 text-xs"><div className="text-slate-500">Находки и ограничения</div><div className="mt-1 text-slate-200">{selectedPhoto.flags.length ? selectedPhoto.flags.join(", ") : selectedPhoto.fuzzy}</div></div><div className="mt-3 flex gap-2"><button onClick={() => selectPhoto(selectedPhoto)} className="rounded border border-cyan-700 px-2 py-1 text-xs text-cyan-200">{aId === selectedPhoto.id ? <><Check className="mr-1 inline h-3 w-3" />A назначен</> : "Назначить A/B"}</button><button className="rounded border border-slate-700 px-2 py-1 text-xs text-slate-300"><ExternalLink className="mr-1 inline h-3 w-3" />Inspector</button></div></aside>}
  </main>;
}
