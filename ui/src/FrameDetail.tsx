  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <article className="detail-modal" onClick={e => e.stopPropagation()}>
      ...
      <div className="detail-grid">
        <div className="detail-image"><img src={`/storage/stage1/${frame.id}/face_crop.jpg`} onError={e => { (e.target as HTMLImageElement).src = `/storage/stage1/${frame.id}/thumb.jpg` }} alt={frame.date} /><p>Crop Stage 1. Цвет рамки не кодирует forensic-вывод.</p></div>
        <div className="detail-data">
          <section><h3>Применимость анализа</h3><Row label="Видимость" value={`${(frame.combinedVisibleFraction * 100).toFixed(1)}%`} /><Row label="Alignment quality" value={value(metrics?.alignmentQuality)} /><Row label="Skin quality" value={value(metrics?.skinQualityScore)} /><Row label="Резкость Laplacian" value={value(metrics?.laplacianVariance, 1)} />{/* V8: poseConfidence удалён — метрика квантована (5 значений), исключена каталогом */}</section>
