...
import {
  ...
  HYPOTHESIS_COLORS, EVENT_PINS, EventPin, FUZZY_COLORS, Hypothesis, PoseBucket,
} from "./data";
import LeftPanel from "./components/LeftPanel";
import Icon from "./components/Icon";
import FullPhotoOverlay from "./components/FullPhotoOverlay";
import ComparisonPanel from "./components/ComparisonPanel";
import { EraCompareView, ClusterView } from "./components/AltViews";
...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<"FULL" | "ERA" | "CLUSTER">("FULL");
  ...
  const [filters, setFilters] = useState<Filters>({
    ...
    flags: new Set(),
    buckets: new Set(["frontal_0", "frontal_yaw15", "frontal_yaw30", "profile_L", "profile_R"] as PoseBucket[]),
    dataset: "main",
    bucketFilter: "all",
    search: "",
  });
...
function HeaderBar({ thumbSize, setThumbSize, filters, setFilters, onSources, onExport, onToggleTheme, theme, viewMode, setViewMode, stats, leftPanelOpen, onToggleLeftPanel, onOpenFilters }: any) {
  return (
    <div data-no-pan className="h-12 bg-surface border-b border-border-strong flex items-center px-3 gap-3 flex-shrink-0">
      ...
      <select value={filters.bucketFilter} onChange={e => setFilters({ ...filters, bucketFilter: e.target.value as any })}
        ...
        <option value="frontal_0">фронт 0°</option>
        <option value="frontal_yaw15">фронт 15°</option>
        <option value="frontal_yaw30">фронт 30°</option>
        <option value="profile_L">профиль слева</option>
        <option value="profile_R">профиль справа</option>
      </select>
