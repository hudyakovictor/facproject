...
from .sampling import select_per_year
...
class Stage1Engine:
    def __init__(self, config: Stage1Config):
        ...
        if self.cfg.sampling_mode == "per_year":
            if not self.cfg.require_filename_date:
                raise ValueError("per_year sampling requires authoritative filename dates")
            self.photos = select_per_year(self.photos, self.cfg.per_year)
