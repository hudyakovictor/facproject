...
def build_merged_timeline(stage1_root: Path, stage2_root: Path | None = None) -> dict[str, Any]:
    """Stage 1 is the authoritative photo set; Stage 2 overlays metrics when present.

    Photos without a Stage 2 pair remain visible with measurementStatus=not_compared.
    Orphan Stage 2 photo IDs are reported separately and never silently injected.
    """
    from .stage1_timeline import build_stage1_inventory

    base = build_stage1_inventory(stage1_root)
    photos = list(base.get("photos") or [])
    by_id = {str(row.get("id")): row for row in photos if row.get("id")}
    stage1_ids = set(by_id)
    compared_ids: set[str] = set()
    orphan_ids: list[str] = []

    if stage2_root is not None and (stage2_root / "analysis_manifest.json").is_file():
        try:
            stage2 = build_research_timeline(stage2_root)
        except (FileNotFoundError, ValueError, KeyError, OSError, json.JSONDecodeError):
            stage2 = None
        if stage2 is not None:
            for row in stage2.get("photos") or []:
                ...
                if photo_id not in by_id:
                    ...
                    continue
                target = by_id[photo_id]
                for key, value in row.items():
                    if key in {"id", "date", "t", "bucket", "era"}:
                        continue
                    if value is not None:
                        target[key] = value
                if not target.get("measurementStatus") or target.get("measurementStatus") == "not_compared":
                    target["measurementStatus"] = row.get("measurementStatus") or "compared"
                flags = list(target.get("flags") or [])
                flags = [flag for flag in flags if flag != "STAGE1_INVENTORY_ONLY"]
                target["flags"] = flags
                target["analysisStage"] = "stage1_plus_stage2"
                compared_ids.add(photo_id)
            if stage2.get("era_meta"):
                base["era_meta"] = stage2["era_meta"]
            if stage2.get("chronology_anomalies") is not None:
                base["chronology_anomalies"] = stage2["chronology_anomalies"]
            base["analysis_manifest"] = stage2.get("analysis_manifest")
            base["note"] = (
                "Полный Stage 1 с наложением доступных результатов Stage 2. "
                "Фотографии без пары сохраняются со статусом not_compared."
            )

    for photo_id, row in by_id.items():
        if photo_id not in compared_ids:
            row["measurementStatus"] = row.get("measurementStatus") or "not_compared"
            row.setdefault("analysisStage", "stage1_inventory")

    base["schema"] = RESEARCH_TIMELINE_SCHEMA
    base["source_mode"] = "research"
    base["not_a_verdict"] = True
    base["photos"] = sorted(
        by_id.values(),
        key=lambda item: (item.get("t") is None, item.get("t") or 0, str(item.get("id") or "")),
    )
    base["stage1_photo_count"] = len(stage1_ids)
    base["stage2_compared_photo_count"] = len(compared_ids)
    base["stage2_orphan_photo_ids"] = sorted(set(orphan_ids))
    base["analysis_stage"] = "merged_stage1_stage2" if compared_ids else base.get("analysis_stage")
    return base
