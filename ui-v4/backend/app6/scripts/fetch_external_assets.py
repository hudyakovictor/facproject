...
def main():
    ...
    p.add_argument('--expected-hashes',required=True,help='JSON mapping filename to approved content digest')
    ...
    if missing: raise SystemExit('missing approved content digest: '+', '.join(missing))
    for n in names:
        q=o/n
        if not q.exists() or a.force:
            ...
            tmp.replace(q)
        got=sha(q)
        if got.lower()!=expected[n].lower(): raise SystemExit(f'existing asset content digest mismatch for {n}: {got}')
        m[n]={'digest':got,'bytes':q.stat().st_size,'source':ASSETS[n]}
