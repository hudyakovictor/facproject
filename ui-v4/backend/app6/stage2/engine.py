...
class Stage2Engine:
 ...
 def run(self):
  ...
  for n,(ptype,a,b,qc_decision) in enumerate(specs,1):
   if self.cfg.cancel_event is not None and self.cfg.cancel_event.is_set():
    raise RuntimeError('run cancelled by operator')
   if self.cfg.progress_callback is not None and n % 10 == 0:
    self.cfg.progress_callback(n,len(specs),'pair_comparison')
